...
import config
import tailscale
# 仮の判断(モジュール分割第一弾・第二弾への対応): web_search関連・
# プロジェクトファイル操作ツール群を`tools.py`に、発見済みピアの共有
# レジストリ・自己紹介カード配信サーバー・mDNS/Tailscale発見を
# `network.py`に切り出した。循環importが生じる定数・ツールスキーマ
# (`tools.py`/`network.py`側の関数がyoriai.py側の関数を必要とする、
# または逆)は`yoriai_types.py`に切り出し、両側からimportしている
