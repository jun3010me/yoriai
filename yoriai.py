...
import config
import tailscale
# 仮の判断(モジュール分割第一弾への対応): web_search関連・プロジェクト
# ファイル操作ツール群を`tools.py`に切り出した。循環importが生じる定数・
# ツールスキーマ(`tools.py`側の関数がyoriai.py側の関数を必要とする、または
# その逆)は`yoriai_types.py`に切り出し、両側からimportしている
# (詳細は各モジュールの冒頭コメントを参照)。ロジックの変更は無い。
...
from tools import (
    ...
    _execute_tool_call,
    _looks_like_tools_related_error,
...
# あるため、読み取りタイムアウトは長めに取る。CHAT_CONNECT_TIMEOUT_SECは
# tools.py側からも参照されるため`yoriai_types.py`で定義している(ファイル
# 冒頭のimportを参照)。
...
# ---------------------------------------------------------------------------
#
# 仮の判断(モジュール分割第一弾への対応): WEB_SEARCH_TOOL_NAME・
...
# 仮の判断(モジュール分割第一弾への対応): _TOOLS_UNSUPPORTED_ERROR_KEYWORDS・
# _looks_like_tools_related_error・WEB_SEARCH_MAX_RESULTS・WEB_SEARCH_TIMEOUT_SEC・
# SEARXNG_BASE_URL・web_search・_execute_tool_callは`tools.py`に移動した
# (ロジック変更なし)。_looks_like_tools_related_errorはTOOLS_UNSUPPORTED_STATUS_CODES
# と組み合わせてこのすぐ下で使われるため、ファイル冒頭でtoolsからimportしている。
