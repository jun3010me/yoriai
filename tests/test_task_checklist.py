...
def test_queue_ensures_all_tasks_complete_even_with_fewer_members_than_tasks():
    ...
    try:
        ...
        project_dir = os.path.join(out_dir, "projects", yoriai._project_name_with_date_prefix("4つのモジュールに分かれたツールを作って"))
        # 仮の判断(チェックポイント運用への対応): `.git`はチェックポイント用の
        # リポジトリであり、生成物の一部ではないため比較対象から除外する。
        saved_files = (set(os.listdir(project_dir)) - {".git"}) if os.path.isdir(project_dir) else set()
...
def test_complete_plan_reports_all_tasks_done():
    ...
    try:
        ...
        # 仮の判断(チェックポイント運用への対応): `.git`はチェックポイント用の
        # リポジトリであり、生成物の一部ではないため比較対象から除外する。
        assert set(os.listdir(project_dir)) - {".git"} == {"storage.py", "cli.py", "utils.py", "config.py", "PROGRESS.md"}
    finally:
        yoriai._fetch_org_snapshot = original_snapshot
        yoriai._stream_chat_from_candidate = original_stream
        shutil.rmtree(out_dir, ignore_errors=True)
